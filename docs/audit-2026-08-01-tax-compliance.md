...
购物车在结账流程中**每次请求都会重算**，而以下三条 error 日志是按次记录的，没有做去重：

- gross 渠道让位（S2-1）
- 省份识别失败回退默认省（S1-1）
- 税率配置无法解析（S1-3）

也就是说，如果商家把某个渠道**长期**配置成含税价，或者某个客户的地址长期缺省份，日志会**每个请求一条**地增长。

这是有意选择：这三种情况都是"必须立刻修正的配置错误"，不该被降噪掩盖；而跨请求去重需要引入
状态（缓存/会话），复杂度和误判风险都高于收益。**但如果你打算长期运行 gross 渠道，请先做日志
轮转或监控告警**，否则磁盘会被填满。
